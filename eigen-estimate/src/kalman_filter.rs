...
pub struct KalmanFilter<Scalar, const DIM: usize, StateTransition, ProcessCovariance>
where
    Scalar: nalgebra::RealField + nalgebra::Scalar,
...
impl<Scalar, const DIM: usize, StateTransition, ProcessCovariance>
    KalmanFilter<Scalar, DIM, StateTransition, ProcessCovariance>
where
    ...
    ProcessCovariance: ProcessCovarianceFunctor<Scalar, DIM>,
{
    ...
    pub fn update<const OBSERVATION_DIM: usize>(
        ...
        let innovation_covariance =
            observation_matrix * &self.estimate_covariance * observation_matrix.transpose()
                + observation_covariance;

        let kalman_gain = &self.estimate_covariance
            * observation_matrix.transpose()
            * innovation_covariance.try_inverse().unwrap();
        let state_update_jacobian = &OMatrix::<Scalar, Const<DIM>, Const<DIM>>::identity()
            - &kalman_gain * observation_matrix;

        self.estimate_state += &kalman_gain * innovation_state;
        self.estimate_covariance =
            &state_update_jacobian * &self.estimate_covariance * state_update_jacobian.transpose()
                + &kalman_gain * observation_covariance * kalman_gain.transpose();
